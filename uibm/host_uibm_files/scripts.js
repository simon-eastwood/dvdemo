    ...
    $.fn.jPushMenu = function(customOptions) {
        ...
        if (o.closeOnClickOutside) {
             $(document).click(function(ev) {
                if( ($(ev.target).is('span') && $(ev.target).closest('.has-submenu').length) || $(ev.target).hasClass('preventclick') ){
                    ev.preventDefault();
                    return true;
                }
                jPushMenu.close(o);
        ...
        if (o.closeOnClickLink) {
            $('.cbp-spmenu a').on('click',function(ev) {
                if( ($(ev.target).is('span') && $(ev.target).closest('.has-submenu').length) || $(ev.target).hasClass('preventclick') ){
                    ev.preventDefault();
                    return true;
                }
...
;( function( window ) {
	...
	UISearch.prototype = {
		...
		open : function() {
			// HACK TO CLOSE ACTIVE JPUSHMENU
			$('.cbp-close').trigger('click');
...
$(document).ready(function() { 
  ...
  if ($('.user-feedback').find('input[name="id"]').length && getCookie('userFeedback') && jQuery.parseJSON(getCookie('userFeedback')).indexOf($('.user-feedback').find('input[name="id"]').val()) != -1) {
    $('.user-feedback-wrapper').remove();
  }else{
    $('.user-feedback-wrapper').show();
  }

  $('.user-feedback form').ajaxForm({
    ...
    error: function(){
      ...
      $('.user-feedback-out').html('<h4><i class="fa fa-exclamation-triangle fa-2x" aria-hidden="true"></i> Si è verificato un errore imprevisto<small>Ti preghiamo di riprovare più tardi.</small></h4>').fadeIn();
    }
  });
...
$(document).ready(function() { 
  ...
  /* With pagination */
  $('.section_datepicker[data-target="pagination"] .filters_btns a').on('click', function(ev){
    ev.preventDefault();
    var $this = $(this);
    var scrollTop = $('.section_datepicker').is(':visible') ? 
    ($('.section_datepicker').offset().top + $('.section_datepicker').height()) :
    ($('.list-wrapper').offset().top - $('.list-wrapper').closest('.section').css('padding-top').replace(/px/, ''));
    $('.filters_btns a').removeClass('active');
    $this.addClass('active');
    $.ajax({
      url: $this.attr('href'),
  ...
  $('.section_datepicker[data-target="pagination"] form').ajaxForm({
    ...
    success: function(responseText, statusText, xhr, formEl){
      ...
      if(!$('.views-row', '.list-wrapper').length) scrollTop -= 50;
      var url = this.url;
      $('html, body').stop().animate({
        scrollTop: scrollTop
      }, 500, function(){
...
$(document).ready(function() { 
  ...
  $(document).on('click', 'ul.pagination:not(".non-ajax") a[href]', function(ev){
    ev.preventDefault();
    var $this = $(this);
    ...
    $.ajax({
      ...
      $('.list-wrapper').css({opacity: 0}).html(response);
      $('html, body').stop().animate({
        scrollTop: scrollTop
      }, 1000, function(){
        history.pushState(null, null, removeURLParameter(url, 'task'));
        $('.list-wrapper').animate({opacity: 1})
      });
    });
  })
...
$(document).on('click', 'a[href^="#"]', function(ev){
  ...
  if(!targetObj.length) return false;
  $('html, body').stop().animate({
...
$(document).ready(function($) {
  ...
  $(document).on('click', '.navmenu li.has-submenu > a', function(ev){
    ...
    if($(ev.target).is('a') && !$(ev.target).hasClass('preventclick')) return true;
    ev.preventDefault();
...
$(document).ready(function($) {
  ...
  if($('.mobile-header-menu ul').length){
    $('.section-nav .toggle-menu-related').css({display: 'inline-block'});
  }
